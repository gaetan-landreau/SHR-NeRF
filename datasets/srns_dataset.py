...
class SRNsDataset(Dataset):
    def __init__(
        ...
        self.mode = mode
        self.scene = scene
        self.all_intrinsics_files = sorted(
            glob.glob(os.path.join(self.folder_path, "*", "intrinsics.txt"))
        )
        ...
        if specific_observation_idx is not None:
            self.specific_observation_idx = parse_comma_separated_integers(
