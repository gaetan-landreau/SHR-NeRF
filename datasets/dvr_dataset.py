...
class DVRDataset(torch.utils.data.Dataset):
    def __init__(
        ...
        categories = [
            x for x in glob.glob(os.path.join(self.base_dir, "*")) if os.path.isdir(x)
        ]
        obj_lists = [
            os.path.join(x, list_prefix + "{}.lst".format(mode)) for x in categories
        ]
        ...
        for obj_list in obj_lists:
            ...
            with open(obj_list, "r") as f:
                objs = [
                    (category, os.path.join(category_dir, x.strip()))
        ...
        print(
            "[Info] Loading DVR dataset: {}, mode: {}, "
            "type: {}, {} objects in total".format(
                self.base_dir, mode, "ShapeNet", len(self.all_objs)
            )
        )
    ...
    def __getitem__(self, index):
        ...
        rgb_paths = [
            x
            for x in glob.glob(os.path.join(obj_dir, "image", "*"))
