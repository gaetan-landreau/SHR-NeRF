...
if __name__ == "__main__":
    ...
    if args.eval_dataset == "srns_dataset":
        out_scene_dir = os.path.join(
            eval_folder, "{}_{:06d}".format(scene_name, model.start_step)
        )
    else:
    ...
    if args.eval_dataset == "srns_dataset":
        test_dataset = dataset_dict[args.eval_dataset](
            args, "test", scene=args.eval_scene
        )
    elif args.eval_dataset == "dvr":
        test_dataset = dataset_dict[args.eval_dataset](
            args, "test", list_prefix="softras_"
        )
    else:
    ...
    for i, data in enumerate(test_loader):
        ...
        if args.multicat:
            ...
            if cat != data["category"][0]:
                if cat != None:
                    cat_results_dict[cat] = {
                        "coarse_psnr": cat_sum_coarse_psnr / cat_total_num,
        ...
        with torch.no_grad():
            ...
            for render_view in render_views:
                ...
                coarse_err_map = torch.sum(
                    (coarse_pred_rgb - gt_rgb) ** 2, dim=-1
                ).numpy()
                coarse_err_map_colored = (
                    colorize_np(coarse_err_map, range=(0.0, 1.0)) * 255
                ).astype(np.uint8)
                imageio.imwrite(
                    os.path.join(out_dir, "{}_err_map_coarse.png".format(file_id)),
                    coarse_err_map_colored,
                )
                coarse_pred_rgb_np = np.clip(
                    coarse_pred_rgb.numpy(), a_min=0.0, a_max=1.0
                )
                ...
                src_rgb_np_uint8 = (
                    255
                    * np.clip(
                        ray_batch["src_img"][0].permute(1, 2, 0).detach().cpu().numpy(),
                        a_min=0,
                        a_max=1.0,
                    )
                ).astype(np.uint8)
                imageio.imwrite(
                    os.path.join(out_dir, "{}_src_rgb.png".format(file_id)),
                    src_rgb_np_uint8,
                )
                ...
                imageio.imwrite(
                    os.path.join(out_dir, "{}_pred_coarse.png".format(file_id)),
                    coarse_pred_rgb,
                )

                gt_rgb_np_uint8 = (
                    255 * np.clip(gt_rgb.numpy(), a_min=0, a_max=1.0)
                ).astype(np.uint8)
                imageio.imwrite(
                    os.path.join(out_dir, "{}_gt_rgb.png".format(file_id)),
                    gt_rgb_np_uint8,
                )

                coarse_pred_depth = ret["outputs_coarse"]["depth"].detach().cpu()
                imageio.imwrite(
                    os.path.join(out_dir, "{}_depth_coarse.png".format(file_id)),
                    (coarse_pred_depth.numpy().squeeze() * 1000.0).astype(np.uint16),
                )
                coarse_pred_depth_colored = colorize_np(
                    coarse_pred_depth,
                    range=tuple(
                        data["depth_range"][0][render_view].squeeze().cpu().numpy()
                    ),
                )
                imageio.imwrite(
                    os.path.join(out_dir, "{}_depth_vis_coarse.png".format(file_id)),
                    (255 * coarse_pred_depth_colored).astype(np.uint8),
                )
                coarse_acc_map = (
                    torch.sum(ret["outputs_coarse"]["weights"], dim=-1).detach().cpu()
                )
                coarse_acc_map_colored = (
                    colorize_np(coarse_acc_map, range=(0.0, 1.0)) * 255
                ).astype(np.uint8)
                imageio.imwrite(
                    os.path.join(out_dir, "{}_acc_map_coarse.png".format(file_id)),
                    coarse_acc_map_colored,
                )
                ...
                running_mean_coarse_lpips = (
                    sum_coarse_lpips / (i + 1) / len(render_views)
                )
                ...
                if ret["outputs_fine"] is not None:
                    ...
                    fine_psnr = metrics.peak_signal_noise_ratio(
                        fine_pred_rgb_np, gt_rgb_np, data_range=1
                    )
                    fine_ssim = metrics.structural_similarity(
                        fine_pred_rgb_np, gt_rgb_np, multichannel=True, data_range=1
                    )
                    fine_lpips = lpips_vgg(
                        fine_pred_rgb[None, ...].permute(0, 3, 1, 2).float().to(device),
                        gt_rgb[None, ...].permute(0, 3, 1, 2).float().to(device),
                    ).item()

                    fine_err_map = torch.sum(
                        (fine_pred_rgb - gt_rgb) ** 2, dim=-1
                    ).numpy()
                    fine_err_map_colored = (
                        colorize_np(fine_err_map, range=(0.0, 1.0)) * 255
                    ).astype(np.uint8)
                    imageio.imwrite(
                        os.path.join(out_dir, "{}_err_map_fine.png".format(file_id)),
                        fine_err_map_colored,
                    )

                    fine_pred_rgb = (
                        255 * np.clip(fine_pred_rgb.numpy(), a_min=0, a_max=1.0)
                    ).astype(np.uint8)
                    imageio.imwrite(
                        os.path.join(out_dir, "{}_pred_fine.png".format(file_id)),
                        fine_pred_rgb,
                    )
                    fine_pred_depth = ret["outputs_fine"]["depth"].detach().cpu()
                    imageio.imwrite(
                        os.path.join(out_dir, "{}_depth_fine.png".format(file_id)),
                        (fine_pred_depth.numpy().squeeze() * 1000.0).astype(np.uint16),
                    )
                    fine_pred_depth_colored = colorize_np(
                        fine_pred_depth,
                        range=tuple(
                            data["depth_range"][0][render_view].squeeze().cpu().numpy()
                        ),
                    ...
                    imageio.imwrite(
                        os.path.join(out_dir, "{}_depth_vis_fine.png".format(file_id)),
                        (255 * fine_pred_depth_colored).astype(np.uint8),
                    )
                    fine_acc_map = (
                        torch.sum(ret["outputs_fine"]["weights"], dim=-1).detach().cpu()
                    )
                    fine_acc_map_colored = (
                        colorize_np(fine_acc_map, range=(0.0, 1.0)) * 255
                    ).astype(np.uint8)
                    imageio.imwrite(
                        os.path.join(out_dir, "{}_acc_map_fine.png".format(file_id)),
                        fine_acc_map_colored,
                    )
                else:
                    fine_ssim = fine_lpips = fine_psnr = 0.0
                ...
                if args.multicat:
                    ...
                    print(
                        ...
                        "=========================================================\n".format(
                            cat,
                            os.path.basename(data["obj_dir"][0]),
                            file_id,
                            coarse_psnr,
                            fine_psnr,
                            running_mean_coarse_psnr,
                            running_mean_fine_psnr,
                            coarse_ssim,
                            fine_ssim,
                            running_mean_coarse_ssim,
                            running_mean_fine_ssim,
                            coarse_lpips,
                            fine_lpips,
                            running_mean_coarse_lpips,
                            running_mean_fine_lpips,
                        )
                    )
                else:
                    print(
                ...
                if args.multicat:
                    ...
                    results_dict[scene_name][obj][file_id] = {
                        "coarse_psnr": coarse_psnr,
                        "fine_psnr": fine_psnr,
                        "coarse_ssim": coarse_ssim,
                        "fine_ssim": fine_ssim,
    ...
    if args.multicat:
        ...
        for cat in cat_results_dict.keys():
            print(
                ...
                "final coarse lpips: {}, final fine lpips: {} \n".format(
                    cat,
                    cat_results_dict[cat]["coarse_psnr"],
                    cat_results_dict[cat]["fine_psnr"],
                    cat_results_dict[cat]["coarse_ssim"],
                    cat_results_dict[cat]["fine_ssim"],
                    cat_results_dict[cat]["coarse_lpips"],
                    cat_results_dict[cat]["fine_lpips"],
                )
            )
    ...
    if args.eval_dataset == "srns_dataset":
        f = open(
            "{}/metrics_{}_{}.txt".format(eval_folder, save_prefix, model.start_step),
            "w",
        )
    else:
