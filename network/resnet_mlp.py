...
class PosEncodeResnet(torch.nn.Module):
    def __init__(self, args, pos_size, x_size,
        ...
        input_size = (
            pos_size * (2 * self.args.multires + 1)
            + x_size
        )
    ...
    def forward(self, pos_x, in_x):
        ...
        Args:
            ...
            in_x: input NOT to be encoded with positional encodings = the direction. 
        """
        x = self.posenc(pos_x) # [N_rays, N_samples, 63]
        x = torch.cat([x, in_x], axis=-1)   # [N_rays, N_samples, 63+512+3+3]
